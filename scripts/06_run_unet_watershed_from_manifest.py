...
DEFAULT_CONFIG_PATH = Path("configs/unet_watershed.yaml")
DEFAULT_MANIFEST_PATH = Path("data/interim/monuseg_manifest.csv")
DEFAULT_OUTPUT_ROOT = Path("outputs/predictions/unet_watershed_manifest")


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run U-Net + Watershed inference from a manifest CSV."
    )
    parser.add_argument(
        "--manifest",
        type=Path,
    ...
    parser.add_argument(
        ...
        help="Path to U-Net + Watershed YAML config.",
    )
    parser.add_argument(
        "--split",
        type=str,
...
def build_runtime_row(
    ...
    return {
        ...
        "tile_size": str(metadata.get("tile_size", "")),
        "threshold": str(metadata.get("threshold", "")),
        "min_size": str(metadata.get("min_size", "")),
        "min_distance": str(metadata.get("min_distance", "")),
...
def main() -> None:
    ...
    runtime_log_path = output_dir / "unet_watershed_runtime.csv"

    ensure_dir(output_dir)

    wrapper = UNetWatershedModelWrapper(config_path=args.config)
    ...
    print("Running U-Net + Watershed from manifest")
    ...
    print("U-Net + Watershed manifest run finished.")
