...
DEFAULT_CONFIG_PATH = Path("configs/cellpose.yaml")
DEFAULT_MANIFEST_PATH = Path("data/interim/monuseg_manifest.csv")
DEFAULT_OUTPUT_ROOT = Path("outputs/predictions/cellpose_manifest")
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run Cellpose inference from a manifest CSV."
    )
    parser.add_argument(
        "--manifest",
        type=Path,
    ...
    parser.add_argument(
        ...
        help="Path to Cellpose YAML config.",
    )
    parser.add_argument(
        "--split",
        type=str,
...
def load_manifest_rows(manifest_path: Path, split: str, limit: int | None) -> list[dict]:
...
def main() -> None:
    ...
    wrapper = CellposeModelWrapper(config_path=args.config)
    ...
    print("Running Cellpose from manifest")
    ...
    print("Cellpose manifest run finished.")
